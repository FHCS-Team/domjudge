...
class ProblemController extends AbstractRestController implements QueryObjectTransformer
{
    ...
    /**
     * Get rubrics for a specific problem.
     *
     * @return array<array{name: string, weight: float, type: string, threshold?: float, description?: string}>
     */
    #[IsGranted(new Expression("is_granted('ROLE_JURY') or is_granted('ROLE_JUDGEHOST')"))]
    #[Rest\Get('/{id}/rubrics')]
    #[OA\Response(
        response: 200,
        description: 'The rubrics for the specified problem',
        content: new OA\JsonContent(
            type: 'array',
            items: new OA\Items(
                properties: [
                    new OA\Property('name', type: 'string', description: 'Name of the rubric'),
                    new OA\Property('weight', type: 'number', description: 'Weight of this rubric (0.0-1.0)'),
                    new OA\Property('type', type: 'string', description: 'Type of rubric (manual/automated)'),
                    new OA\Property('threshold', type: 'number', nullable: true, description: 'Pass threshold for this rubric'),
                    new OA\Property('description', type: 'string', nullable: true, description: 'Description of the rubric'),
                ]
            )
        )
    )]
    #[OA\Parameter(ref: '#/components/parameters/id')]
    public function getProblemRubricsAction(Request $request, string $id): array
    {
        $contestId = $this->getContestId($request);
        
        // Find the contest problem by external ID
        $contestProblem = $this->em->getRepository(ContestProblem::class)
            ->findOneBy([
                'contest' => $contestId,
                'problem' => $this->em->getRepository(Problem::class)->findOneBy(['externalid' => $id])
            ]);
            
        if (!$contestProblem) {
            throw new NotFoundHttpException("Problem with ID '$id' not found in contest $contestId");
        }
        
        $problem = $contestProblem->getProblem();
        
        // Get all rubrics for this problem
        $rubrics = $this->em->getRepository(Rubric::class)
            ->findBy(['problem' => $problem], ['name' => 'ASC']);

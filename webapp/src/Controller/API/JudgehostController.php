...
class JudgehostController extends AbstractFOSRestController
{
    ...
    #[Rest\Post('/judgehosts/{hostname}/judgings')]
    #[OA\Post(
        summary: 'Submit rubric-based scores and feedback for a submission',
        requestBody: new OA\RequestBody(
            required: true,
            content: new OA\JsonContent(
                properties: [
                    new OA\Property('submission_id', type: 'integer', description: 'ID of the submission'),
                    new OA\Property(
                        'scores',
                        type: 'array',
                        items: new OA\Items(
                            properties: [
                                new OA\Property('rubric', type: 'string', description: 'Rubric name or ID'),
                                new OA\Property('type', type: 'string', enum: ['manual', 'automated'], description: 'Type of scoring'),
                                new OA\Property('judge_id', type: 'integer', nullable: true, description: 'ID of manual judge'),
                                new OA\Property('judgehost_id', type: 'integer', nullable: true, description: 'ID of judgehost (for automated)'),
                                new OA\Property('score', type: 'number', minimum: 0, maximum: 1, description: 'Score between 0.0 and 1.0'),
                                new OA\Property('feedback', type: 'string', nullable: true, description: 'Optional feedback/comments'),
                            ]
                        )
                    ),
                    new OA\Property('aggregate_score', type: 'number', description: 'Overall aggregate score'),
                    new OA\Property('logs', type: 'string', nullable: true, description: 'Execution logs'),
                    new OA\Property('timestamp', type: 'string', format: 'date-time', description: 'Timestamp when judging completed'),
                ]
            )
        ),
        responses: [
            new OA\Response(
                ...
                content: new OA\JsonContent(
                    properties: [
                        ...
                        new OA\Property('scores_processed', type: 'integer'),
                    ]
                )
            ),
    ...
    public function submitJudgingScores(
        Request $request,
        #[OA\PathParameter(description: 'The hostname of the judgehost submitting scores')]
        string $hostname
    ): JsonResponse {
        $judgehost = $this->em->getRepository(Judgehost::class)->findOneBy(['hostname' => $hostname]);
        if (!$judgehost) {
        ...
        $data = json_decode($request->getContent(), true);
        if (!$data || !isset($data['submission_id']) || !isset($data['scores'])) {
            throw new BadRequestHttpException('Invalid request data. Required: submission_id, scores');
        }

        $submissionId = (int)$data['submission_id'];
        $scores = $data['scores'];
        $aggregateScore = $data['aggregate_score'] ?? null;
        $logs = $data['logs'] ?? null;
        $timestamp = $data['timestamp'] ?? null;

        // Find the submission
        $submission = $this->em->getRepository(Submission::class)->find($submissionId);
        if (!$submission) {
            throw new NotFoundHttpException("Submission with ID $submissionId not found");
        }

        // Verify this is a hackathon submission
        if (!$submission->getContest()->getHackathonEnabled()) {
            throw new BadRequestHttpException('Submission is not from a hackathon-enabled contest');
        }

        $scoresProcessed = 0;
        $judgeHostname = $judgehost->getHostname();

        foreach ($scores as $scoreData) {
            if (!isset($scoreData['rubric']) || !isset($scoreData['score']) || !isset($scoreData['type'])) {
                continue; // Skip invalid score entries
            }

            $rubricName = $scoreData['rubric'];
            $score = (float)$scoreData['score'];
            $type = $scoreData['type'];
            $feedback = $scoreData['feedback'] ?? null;

            // Find rubric by name or ID
            $rubric = null;
            if (is_numeric($rubricName)) {
                $rubric = $this->em->getRepository(Rubric::class)->find((int)$rubricName);
            } else {
                $rubric = $this->em->getRepository(Rubric::class)->findOneBy([
                    'name' => $rubricName,
                    'problem' => $submission->getProblem()
            ...
            if (!$rubric) {
                continue; // Skip if rubric not found
            }
            ...
            // Set judge name based on type
            $judgeName = $type === 'automated' ? "judgehost-{$judgeHostname}" : "judge-{$scoreData['judge_id']}";
            
            $rubricScore->setScore($score);
            $rubricScore->setJudgeName($judgeName);
            $rubricScore->setJudgedAt(new \DateTime($timestamp ?? 'now'));
            $rubricScore->setComments($feedback);

            $scoresProcessed++;
        }

        // Store aggregate score and logs if needed (could extend SubmissionRubricScore or create separate table)
        // For now, we'll just log it
        if ($aggregateScore !== null || $logs !== null) {
            $this->dj->auditlog('judging', $submission->getSubmitid(), 'hackathon-scores', [
                'aggregate_score' => $aggregateScore,
                'logs' => $logs,
                'judgehost' => $judgeHostname,
                'timestamp' => $timestamp,
            ]);
        }
        ...
        return new JsonResponse([
            'success' => true,
            'message' => "Processed {$scoresProcessed} rubric scores for submission {$submissionId}",
            'scores_processed' => $scoresProcessed,
        ]);
    }
}

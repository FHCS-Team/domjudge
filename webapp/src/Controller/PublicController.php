...
class PublicController extends BaseController
{
    ...
    // Ensure templates that expect `contest` (legacy) receive it as well. Twig strict mode
    // may be enabled in dev which causes an exception when `contest` is referenced but
    // not defined. Provide both keys to be safe.
    $data['contest'] = $contest;

        // Check if hackathon mode is enabled and get display data
        $hackathonDisplayData = null;
        if ($contest && $contest->getHackathonEnabled()) {
            $hackathonDisplayData = $this->em->getRepository(ContestDisplayData::class)

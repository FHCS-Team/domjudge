...
class ProblemController extends BaseController
{
    ...
    public function problemsAction(): Response
    {
        ...
        // Add hackathon display data if hackathon mode is enabled
        if ($contest && $contest->getHackathonEnabled()) {
            $problemDisplayData = [];
            foreach ($data['problems'] as $problem) {
                $displayData = $this->em->getRepository(ProblemDisplayData::class)
                    ->findOneBy(['problem' => $problem->getProblem()]);
                if ($displayData) {
                    $problemDisplayData[$problem->getProblem()->getProbid()] = $displayData;
                }
            }
            $data['problemDisplayData'] = $problemDisplayData;
        }
    ...
    protected function getBinaryFile(int $probId, callable $response): StreamedResponse
    {
        ...
        // In hackathon mode, allow access to problem statements even before contest starts
        $isHackathon = $contest && $contest->getHackathonEnabled();
        $contestAccessible = $contest && ($contest->getFreezeData()->started() || $isHackathon);
        
        if (!$contestAccessible) {
